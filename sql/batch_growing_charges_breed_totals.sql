-- Growing charges: detail + Sub Total (per breed) + Grand Total
-- Written in the same style as the cage-wise report:
--   middle query : GROUP BY ROLLUP + label + DISTINCT
--   outer query  : span / tot analytics over the filtered rows
--   filter       : (label = '-' and detail cols not null) or label like '%Total%'
--
-- label     : '-' = detail row, 'Sub Total' = breed total, 'Grand Total' = report total
-- breedspan : rowspan for the breed cell (number of detail rows under the breed)
-- color_grp : 1 = detail, 0 = Sub Total, -1 = Grand Total (for report coloring)

select label,
       count(case when label = '-' then branch_name end)
             over(partition by breed) as breedspan,
       sum(case when label = '-' then no_of_farms else 0 end)
             over(partition by breed) as tot,
       zone,
       region,
       region_code,
       branch_name,
  ...
  from (select distinct
               case
                 when breed is not null and branch_name is null then 'Sub Total'
                 when breed is null                             then 'Grand Total'
                 else '-'
               end                                                                label,
               zone,
               region,
               ...
               no_of_farms,
               case
                 when breed is null       then -1
                 when branch_name is null then 0
                 else 1
               end                                                                color_grp,
               ...
               sug_gn.round_x(return_feed_perc, 1)                                return_feed_perc,
               nvl(zone_seq, 9)                                                   zone_seq,
               sug_gn.round_2(avg_sales_price)                                    avg_sales_price,
               sug_gn.round_2(feed_consumed)                                      feed_consumed,
               sug_gn.round_2(no_of_birds_sold)                                   no_of_birds_sold,
               sug_gn.round_2(mean_age_new)                                       mean_age_new,
               sug_gn.round_2(birds_lifted)                                       birds_lifted,
               sug_gn.round_2(lifted_wt)                                          lifted_wt,
               sug_gn.round_2(mort)                                               mort
          from (select nvl(b.opm_zone, 'NA')                                          zone,
                       max(nvl(b.zone_seq, '9'))                                      zone_seq,
                       b.region,
                       b.region_code,
                       b.branch_name,
                       b.location_name,
                       a.breed,
                       sum(a.feed_consumed)                                           feed_consumed,
                       ...
                       round((avg(a.feed_consumed) / avg(a.sold_kgs)) +
                             (2 - (avg(a.sold_kgs) / avg(a.sold_no))) / 0.04 * 0.01,
                             2)                                                       cfcr,
                       round(avg(a.sold_kgs) / avg(a.sold_no), 2)                     avg_wt,
                       round((avg(a.mean_age_new * a.sold_no)) / avg(a.sold_no))      mean_age,
                       round(((sum(a.lifted_wt) / sum(a.birds_lifted)) /
                             ((avg(a.mean_age_new * a.sold_no) / avg(a.sold_no)))) * 1000,
                             2)                                                       day_gain,
                       ...
                       sum(a.sold_kgs * a.avg_selling_price) / sum(a.sold_kgs)        avg_sales_price,
                       sum(a.mean_age_new)                                            mean_age_new,
                       sum(a.birds_lifted)                                            birds_lifted,
                       sum(a.lifted_wt)                                               lifted_wt,
                       avg(a.mort)                                                    mort,
                       avg(a.mort7)                                                   mort7
                 ...
                 group by rollup(a.breed,
                                 (b.opm_zone,
                                  ...
                                  b.location_name))))
 where 1 = 1
   and ((label = '-' and breed is not null and branch_name is not null)
        or label like '%Total%')
 order by case when label = 'Grand Total' then 2 else 1 end,
          breed,
          case when label = 'Sub Total' then 2 else 1 end,
          zone_seq,
          branch_name,
          no_of_farms desc;
