-- Batch growing charges report with detail, breed total, and grand total rows.
-- color_grp: 1 = detail, 0 = breed total, -1 = grand total
...
select zone,
       ...
       no_of_farms,
       case
         when grp_breed = 1 then -1          -- grand total
         when grp_branch = 1 then 0          -- breed total
         else 1                              -- detail
       end                                                                   as color_grp,
  ...
  from (select nvl(b.opm_zone, 'NA')                                                     as zone,
               b.zone_seq,
               b.region,
               b.region_code,
               case
                 when grouping(a.breed) = 1 then 'Grand Total'
                 when grouping(b.branch_name) = 1 then 'Breed Total'
                 else b.branch_name
               end                                                                       as branch_name,
               case
                 when grouping(b.location_name) = 1 then null
                 else b.location_name
               end                                                                       as location_name,
               case
                 when grouping(a.breed) = 1 then null
                 else a.breed
               end                                                                       as breed,
               grouping(a.breed)                                                         as grp_breed,
               grouping(b.branch_name)                                                   as grp_branch,
               ...
               sug_gn.round_2(avg(mc) / avg(sold_kgs))                                   as med_cost,
 ...
 order by breed nulls last,
          color_grp desc,             -- detail (1), breed total (0), grand total (-1)
          nvl(zone_seq, 9),
          branch_name,
          no_of_farms desc;
